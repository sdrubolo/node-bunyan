...
// The 'fs-extra' module is required for rotating-file stream support.
try {
...
Logger.prototype.addStream = function addStream(s, defaultLevel) {
    ...
    case 'rotating-file':
        ...
        assert.ok(s.path);
        assert.ok(fs_extra, '"rotating-file" stream type is not supported: '
    ...
    function moves() {
        ...
        fs.exists(before, function (exists) {
            if (!exists) {
                ...
                self._debug('  fs %s %s', before, after);
                fs_extra.move(before, after, function (err) {
                    if (err) {
                        self.emit('error', err);
